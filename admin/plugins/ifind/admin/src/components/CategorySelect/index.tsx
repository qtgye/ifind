...
import {
  ...
  mapCategoriesTree,
  buildCategoryPath as _buildCategoryPath,
...
const CategorySelect = ({
  ...
  const [categoryOptions, setCategoryOptions] = useState<CategoryWithChild[]>(
    []
  ); // source-region-filtered categories
  ...
  const onNestedCategoryOptionChange = useCallback(
    (lastCategory) => {
  ...
  useEffect(() => {
    ...
    const filteredOptions = Object.values(categoryTree).filter(
      (category) => exclude && !exclude.includes(category.id)
    );
