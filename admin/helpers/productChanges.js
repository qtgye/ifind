        ...
        return (
          ...
          changedUrlList.some((changedURLData, index) => {
            ...
            return (
              ...
              // TODO: remove usage of Region. Replace with UrlWithType.region instead?
              // originalURLData.region.id != changedURLData.region ||
              originalURLData.merchant != changedURLData.merchant ||
      ...
      case "attrs_rating":
        ...
        return (
          ...
          originalAttrsRating.some((originalAttrRating) => {
            const matchedChangedRating = changedAttrsRating.find(
              ({ id }) => id == originalAttrRating.id
            );

            // Attribute must have been removed on change
            if (!matchedChangedRating) {
              return true;
            }
            return compareAttributeChanges(
              originalAttrRating,
              matchedChangedRating
            );
          })
        );
      case "title":
